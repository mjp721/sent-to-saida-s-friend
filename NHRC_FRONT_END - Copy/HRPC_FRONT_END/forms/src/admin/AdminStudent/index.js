...
const AdminStudent = () => {
    ...
    const adminStudentDeleteBtn = async (id) => {
        const url = `http://127.0.0.1:8000/api/student/delete/${id}/`
        const options = {
            method: 'DELETE'
        }
        const response = await fetch(url, options)
        const updatedData = studentList?.filter((each) => each.id !== id)
    ...
    const columns = [
        ...
        {
            selector: row => (
                <Link to={`/student/${row.id}`} className="view-link-btn">
                    View
                </Link>
            ),
            width: "85px",
        ...
        {
            selector: row => (
                <button type="button" onClick={() => { adminStudentDeleteBtn(row.id) }} className="btn btn-danger btn-sm ">DELETE</button>
            ),
            width: "120px",
        },
    ];
    ...
    const userSearchInput1 = (event) => {
        setUserSearch1(event.target.value)
    }

    const studentFilterd = studentList?.filter((each) => {
    ...
    const filterbyCategories = studentFilterd?.filter((item) => {
        if (categories === 'fullName') {
            return userSearch1 === undefined ? item : item.fullName?.toLowerCase().includes(userSearch1?.toLowerCase())
        }
        else if (categories === 'qualification') {
            return userSearch1 === undefined ? item : item.qualification?.toLowerCase().includes(userSearch1?.toLowerCase())
        }
        else if (categories === 'personalMail') {
            return userSearch1 === undefined ? item : item.personalMail?.toLowerCase().includes(userSearch1?.toLowerCase())
        }
        else if (categories === 'state') {
            return userSearch1 === undefined ? item : item.state?.toLowerCase().includes(userSearch1?.toLowerCase())
        }
        else if (categories === 'mobileNumber') {
            return userSearch1 === undefined ? item : item.mobileNumber?.includes(userSearch1)
        }
        return item
    })
        ...
        <div className="admin-db-container d-flex">
            ...
            <div className="col-lg-10 admin-db-main-items-container mt-5 mt-lg-0 pt-4">
                <div className="d-md-flex justify-content-between align-items-center  mt-3 mt-lg-0">
                    <h4 className="mt-1"><b>Student List</b></h4>
                    <div className="d-md-flex">
                        ...
                        <select className="categories-filed mx-2 my-1 my-lg-0 p-1 p-lg-0" value={categories} onChange={(e) => {
                            ...
                            <option value='personalMail'>Mail</option>
                            <option value='state'>State</option>
                            <option value='mobileNumber'>MobileNumber</option>
                        </select>
                        <div className="admin-hr-search-input-container d-flex">
                            <input type="search" onChange={userSearchInput1} value={userSearch1} className="admin-hr-search-input w-100 h-100" placeholder="Search.." alt="img" />
